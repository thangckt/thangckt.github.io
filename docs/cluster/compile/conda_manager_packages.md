...
!!! note
    ...
    - ucx conda does not work. Maybe compile ucx separately.
...
conda install -y --update-specs -c conda-forge python=3.11.4 "gpaw=*=*openmpi*" "ucx=*=*cpu" "openmpi=*=h*" # lammps
```
...
``` sh
module load conda/conda3
conda  create -y -n py11gpaw python=3.11
source activate py11gpaw
```
